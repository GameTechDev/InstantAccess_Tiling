...
D3D11_MAPPED_SUBRESOURCE CPUTTextureDX11::MapTexture( CPUTRenderParameters &params, eCPUTMapType type, bool wait )
{
    ...
    {
        ASSERT( mMappedType == type || (mMappedType == CPUT_MAP_READ_WRITE), _L("Mapping with a different CPU access than creation parameter.") );
    }
...
void CPUTTextureDX11::GetDesc(D3D11_TEXTURE2D_DESC *pdesc)
{
    D3D11_RESOURCE_DIMENSION dim;
    mpTexture->GetType(&dim);
    if(dim == D3D11_RESOURCE_DIMENSION_TEXTURE2D)
    {
        ((ID3D11Texture2D*)mpTexture)->GetDesc(pdesc);
    }
}