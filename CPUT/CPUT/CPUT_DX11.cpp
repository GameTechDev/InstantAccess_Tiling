...
/*
                ...
                typedef HRESULT ( WINAPI*LPDXGIGetDebugInterface )( const GUID, void** );
                LPDXGIGetDebugInterface pDXGIGetDebugInterface = NULL;
                pDXGIGetDebugInterface = ( LPDXGIGetDebugInterface)GetProcAddress( hDXGIDebug, "DXGIGetDebugInterface" );

                IDXGIDebug* pDXGIDebug = NULL;
                pDXGIGetDebugInterface(IID_IDXGIDebug, (void**)(&pDXGIDebug));
    ...
    if( mpPerFrameConstantBuffer )
    {
        ...
        mpContext->Map( pBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapInfo );
        {
            ...
            if( pCamera )
            {
                ...
                pCb->Projection      = projection;
                float4x4 inverseView = inverse(*pCamera->GetViewMatrix());
                pCb->InverseView     = DirectX::XMMATRIX((float*)&inverseView);
				// TODO: ViewProjection matrix belongs here, not in per-Model const buf
				//       Requires coordination as const bufs must match shaders
                // XMMATRIX viewProjection  = view * projection;
                // pCb->ViewProjection  = viewProjection;
            }
            pCb->LightColor         = DirectX::XMLoadFloat3(&DirectX::XMFLOAT3((float*)&mLightColor)); // TODO: Get from light
            float totalSecondsFloat = (float)totalSeconds;
            pCb->TotalSeconds       = DirectX::XMLoadFloat(&totalSecondsFloat);
            pCb->AmbientColor       = DirectX::XMLoadFloat3(&DirectX::XMFLOAT3((float*)&mAmbientColor));
        }
...
void CPUT_DX11::InnerExecutionLoop()
{
    ...
    if(!mbShutdown)
    {
        ...
        if(CPUTRenderTargetColor::GetActiveHeight() > 64 && CPUTRenderTargetColor::GetActiveWidth() > 64)
			Render(deltaSeconds);
...
void CPUTSetDebugName( void *pResource, cString name )
{
    ...
    if(pResource != NULL)
    {
	    ...
	    if(checkLength == 0)
	    {
		    ...
		    HRESULT hr = ((ID3D11DeviceChild*)pResource)->SetPrivateData( WKPDID_D3DDebugObjectName, (UINT)name.length(), pCharString );
	    }
    }
#endif // _DEBUG
}
