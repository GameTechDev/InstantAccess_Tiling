...
void CPUTModelDX11::UpdateShaderConstants(CPUTRenderParameters &renderParams)
{
    ...
    pContext->Map( mpModelConstantBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapInfo );
    {
        ...
        if( pCamera )
        {
            ...
            DirectX::XMVECTOR determinant     = DirectX::XMMatrixDeterminant(world);
            cb.WorldViewProjection = world * viewProjection;
            cb.InverseWorld        = DirectX::XMMatrixInverse(&determinant, XMMatrixTranspose(world));
        ...
        cb.LightDirection     = DirectX::XMVector3Normalize(lightDirection);
        cb.EyePosition        = cameraPos;
        ...
        cb.BoundingBoxCenterWorldSpace  = DirectX::XMLoadFloat3(&DirectX::XMFLOAT3( bbCWS[0], bbCWS[1], bbCWS[2] )); ;
        cb.BoundingBoxHalfWorldSpace    = DirectX::XMLoadFloat3(&DirectX::XMFLOAT3( bbHWS[0], bbHWS[1], bbHWS[2] )); ;
        cb.BoundingBoxCenterObjectSpace = DirectX::XMLoadFloat3(&DirectX::XMFLOAT3( bbCOS[0], bbCOS[1], bbCOS[2] )); ;
        cb.BoundingBoxHalfObjectSpace   = DirectX::XMLoadFloat3(&DirectX::XMFLOAT3( bbHOS[0], bbHOS[1], bbHOS[2] )); ;
        ...
        if( pShadowCamera )
        {
            ...
            cb.LightWorldViewProjection = world * shadowView * shadowProjection;
        }
		memcpy(pCb, &cb, sizeof(CPUTModelConstantBuffer));
    }
