...
)
{
    ...
    if( mMultiSampleCount>1 )
    {
        ...
        if( !recreate )
        {
            ...
            {
                pServices->ResolveAbsolutePathAndFilename( (pAssetLibrary->GetTextureDirectoryName() + msaaName), &finalName);
            }
...
//-----------------------------------------------------------------------------
void CPUTRenderTargetColor::SaveRenderTargetToFile( CPUTRenderParameters &renderParams, cString fileName, int format )
{
	//ASSERT(0);
    //ID3D11DeviceContext *pContext = ((CPUTRenderParametersDX*)&renderParams)->mpContext;
    //HRESULT hr = D3DX11SaveTextureToFile( pContext, mpColorTextureDX, (D3D11_IMAGE_FILE_FORMAT)format, fileName.c_str() );
   // ASSERT( SUCCEEDED(hr), _L("Failed writing render target to file.") );
}
...
//-----------------------------------------------------------------------------
void CPUTRenderTargetDepth::SaveRenderTargetToFile( CPUTRenderParameters &renderParams, cString fileName, int format )
{
    //ASSERT(0);
	//ID3D11DeviceContext *pContext = ((CPUTRenderParametersDX*)&renderParams)->mpContext;
    //D3DX11SaveTextureToFile( pContext, mpDepthTextureDX, (D3DX11_IMAGE_FILE_FORMAT)format, fileName.c_str() );
}
